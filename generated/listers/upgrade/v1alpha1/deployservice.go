...
type DeployServiceLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.DeployService, err error)
...
// List lists all DeployServices in the indexer.
func (s *deployServiceLister) List(selector labels.Selector) (ret []*v1alpha1.DeployService, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.DeployService))
	})
	return ret, err
}
...
type DeployServiceNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.DeployService, error)
...
// List lists all DeployServices in the indexer for a given namespace.
func (s deployServiceNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.DeployService, err error) {
...
// Get retrieves the DeployService from the indexer for a given namespace and name.
func (s deployServiceNamespaceLister) Get(name string) (*v1alpha1.DeployService, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("deployservice"), name)
	}
	return obj.(*v1alpha1.DeployService), nil
}
